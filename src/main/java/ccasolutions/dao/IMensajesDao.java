...
public interface IMensajesDao extends CrudRepository<Mensaje, Long>
{
	...
	@Query("SELECT m FROM Mensaje m WHERE m.receptorId = :receptorId AND m.estaEnPapelera = false")
    List<Mensaje> findMessagesByReceptorId(@Param("receptorId") Long receptorId);
	

	//Metodo para la bandeja de entrada completa
	@Query("SELECT m FROM Mensaje m WHERE m.emisorId = :emisorId AND m.estaEnPapelera = false")
	...
	@Query("SELECT m FROM Mensaje m WHERE m.emisorId = :emisorId AND m.receptorId = :receptorId AND m.estaEnPapelera = false")
